...
def deal_message(ws, message):
    ...
    # 如果status为2，说明数据已经接收完,可以关闭连接
    if status == 2:
        # 删除txt文件
        ne_utils.remove_file("./resource/input/1.txt")
        # 播放本地音频
        audio = AudioSegment.from_file("./resource/output/audio.lame", format="mp3")
        ...
        stream.close()
        p.terminate()
        ne_utils.remove_file("./resource/output/audio.lame")
        ws.close()
    print('status:{}\n'.format(status))
    # 打印Base64解码后数据并生成文件
